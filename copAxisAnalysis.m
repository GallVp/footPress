function copAxisAnalysis(hFig, copTraj, colorSchemeNum )
...
[m, n] = size(copTraj);             % returns m=rows, n=columns of d
mean_d = nanmean(copTraj);          % returns row vector with column means of d
cov_mtx = cov(copTraj, 'omitrows'); % covariance matrix for d
...
figure(hFig);
hold on
plot(copTraj(:,1),copTraj(:,2),cols{1});  % scatter plot with x=column 1 of d, y=column 2

plot(semimaj(:,1),semimaj(:,2),cols{2},'LineWidth', 2);
plot(semimin(:,1),semimin(:,2) ,cols{2},'LineWidth', 2);
plot(ellipse(:,1),ellipse(:,2) ,cols{3},'LineWidth', 2);
end

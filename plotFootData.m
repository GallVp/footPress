...
vars.pipesMenu = uimenu('Label','Options');
...
vars.txtFrameInfo = uicontrol(...
    'Style','text',...
    'Position',[25 50 125 20]);

vars.txtIntervalInfo = uicontrol(...
    'Style','text',...
    'Position',[25 15 125 20],...
...
%% Processed data function
    function processData(updateCOPTrajectory)
        ...
        % Get and split data
        if(vars.splitFramesFlag)
            vars.processedData.leftFrames = splitMat(vars.inputData.leftFootData.frames,...
            ...
            vars.processedData.rightFrames = splitMat(vars.inputData.rightFootData.frames,...
